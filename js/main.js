...
const CaregoriesAreasIngrediants = document.getElementById(
  'CaregoriesAreasIngrediants'
);
...
function getIngradients(meal) {
  ...
  const ingrediantMap = ingradentVals.map((ingradent, index) => {
    return ingradent
      ? `<span class="alert alert-info p-1 m-2">${ingredientMeeasure[index]} ${ingradent}</span>`
      : null;
  });

  const filtedIngrediants = ingrediantMap
    .filter((ingradient) => ingradient)
    .join(' ');
  return filtedIngrediants;
}

function getTags(tags) {
  const taggedMeal = tags
    ? tags
        .split(',')
        .map((tag) => {
    ...
    for (let i = 0; i < 20; i++) {
      cart += `   <div class="col-md-3 category" id=${data[i].idIngredient} >
                    ...
                    <i class="fa-solid fa-drumstick-bite fa-4x h2-area"></i>
                                ...
                                <p class="h2-area" >${data[i].strDescription
                                  ?.split(' ')
                                  .slice(0, 20)
                                  .join(' ')}</p>
...
function clickElement(url) {
  document
    .getElementById('CaregoriesAreasIngrediants')
    .addEventListener('click', function (e) {
      ...
      getMealsCategoriesAreaIng(
        `https://www.themealdb.com/api/json/v1/1/filter.php?${filterParam}`
      );
    });
}
